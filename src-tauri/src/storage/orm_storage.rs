...
impl Storage {
    ...
    pub async fn new(
        ...
        info!("Initializing SeaORM storage instance, database path: {}, SQL logging: {}, log_level: {:?}",
              database_url, sql_log, log_level);

        let mut options = sea_orm::ConnectOptions::new(database_url);
        options.sqlx_logging(sql_log);
        options.sqlx_logging_level(log_level); // 直接使用传入的 log::LevelFilter
    ...
    pub async fn get_mcp_server(
        &self,
        name: &str,
    ) -> Result<Option<mcp_server::Model>, StorageError> {
    ...
    pub async fn update_mcp_server(
        ...
        active_server.args = Set(config
            .args
            .as_ref()
            .map(|args| serde_json::to_string(args).unwrap_or_default()));
        active_server.url = Set(config.url.clone());
        active_server.headers = Set(config
            .headers
            .as_ref()
            .map(|headers| serde_json::to_string(headers).unwrap_or_default()));
        active_server.env = Set(config
            .env
            .as_ref()
            .map(|env| serde_json::to_string(env).unwrap_or_default()));
    ...
    pub async fn list_server_tools(
        &self,
        server_id: &str,
    ) -> Result<Vec<mcp_tool::Model>, StorageError> {
    ...
    pub async fn list_server_resources(
        &self,
        server_id: &str,
    ) -> Result<Vec<mcp_resource::Model>, StorageError> {
    ...
    pub async fn list_server_prompts(
        &self,
        server_id: &str,
    ) -> Result<Vec<mcp_prompt::Model>, StorageError> {
    ...
    pub async fn get_database_stats(
        ...
        let token_count = token::Entity::find()
            .count(&self.db)
            .await
            .map_err(|e| StorageError::Database(format!("Failed to count tokens: {}", e)))?
            as i64;
        stats.insert("tokens".to_string(), token_count);

        let permission_count = permission::Entity::find()
            .count(&self.db)
            .await
            .map_err(|e| StorageError::Database(format!("Failed to count permissions: {}", e)))?
            as i64;
        stats.insert("permissions".to_string(), permission_count);

        let mcp_server_count = mcp_server::Entity::find()
            .count(&self.db)
            .await
            .map_err(|e| StorageError::Database(format!("Failed to count mcp_servers: {}", e)))?
            as i64;
        stats.insert("mcp_servers".to_string(), mcp_server_count);

        let mcp_tool_count = mcp_tool::Entity::find()
            .count(&self.db)
            .await
            .map_err(|e| {
                StorageError::Database(format!("Failed to count mcp_server_tools: {}", e))
            })? as i64;
        stats.insert("mcp_server_tools".to_string(), mcp_tool_count);

        let mcp_resource_count =
            mcp_resource::Entity::find()
                .count(&self.db)
                .await
                .map_err(|e| {
                    StorageError::Database(format!("Failed to count mcp_server_resources: {}", e))
                })? as i64;
        stats.insert("mcp_server_resources".to_string(), mcp_resource_count);

        let mcp_prompt_count = mcp_prompt::Entity::find()
            .count(&self.db)
            .await
            .map_err(|e| {
                StorageError::Database(format!("Failed to count mcp_server_prompts: {}", e))
            })? as i64;
    ...
    pub async fn get_server_config(
        &self,
        server_name: &str,
    ) -> Result<Option<McpServerConfig>, StorageError> {
    ...
    pub async fn create_token(&self, token: &Token) -> Result<String, StorageError> {
        ...
        let token_model = token::ActiveModel {
            ...
            created_at: Set(now.into()),
            updated_at: Set(now.into()),
            last_used_at: Set(token.last_used_at.map(|ts| {
                chrono::DateTime::from_timestamp(ts as i64, 0)
                    .unwrap_or(now)
                    .into()
            })),
            usage_count: Set(token.usage_count as i32),
            expires_at: Set(token.expires_at.map(|ts| {
                chrono::DateTime::from_timestamp(ts as i64, 0)
                    .unwrap_or(now)
                    .into()
            })),
    ...
    pub async fn get_token_by_id(&self, token_id: &str) -> Result<Option<Token>, StorageError> {
        if let Some(token_entity) = token::Entity::find_by_id(token_id.to_string())
            .one(&self.db)
            .await
            .map_err(|e| StorageError::Database(format!("Failed to query: {}", e)))?
        {
    ...
    pub async fn delete_token(&self, token_id: &str) -> Result<(), StorageError> {
        token::Entity::delete_by_id(token_id.to_string())
            .exec(&self.db)
            .await
            .map_err(|e| StorageError::Database(format!("Failed to delete: {}", e)))?;
    ...
    /// 获取Token的所有权限
    pub async fn get_token_permissions(
        &self,
        token_id: &str,
    ) -> Result<Vec<permission::Model>, StorageError> {
    ...
    pub async fn add_permission(
        ...
        if let Some(permission) = existing_permission {
            ...
            tracing::info!(
                "Permission already exists, updated to allowed: token={}, type={}, path={}",
                token_id,
                resource_type,
                resource_path
            );
    ...
    pub async fn remove_permission(
        ...
        if let Some(permission) = existing_permission {
            ...
            tracing::info!(
                "Permission set to denied state: token={}, type={}, path={}",
                token_id,
                resource_type,
                resource_path
            );
            ...
            tracing::info!(
                "Created denied permission record: token={}, type={}, path={}",
                token_id,
                resource_type,
                resource_path
            );
